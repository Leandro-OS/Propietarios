  ...
  <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      ...
      <div class="bg-gray-50 p-3 rounded">
        ...
        <p class="font-medium">{{ formatDate(comunidad.createdAt) }}</p>
      </div>
      <div class="bg-gray-50 p-3 rounded">
        <span class="text-gray-500 text-sm">Última actualización:</span>
        <p class="font-medium">{{ formatDate(comunidad.updatedAt) }}</p>
      </div>
      <div class="bg-gray-50 p-3 rounded">
        <span class="text-gray-500 text-sm">Nº Actualizaciones:</span>
        <p class="font-medium">{{ comunidad.numActualizaciones }}</p>
      </div>
    </div>

    <div class="flex flex-wrap gap-3 mb-4">
      <button @click="$emit('modify')" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition">
        MODIFICAR COMUNIDAD
      </button>
      <button @click="$emit('delete')" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition">
        SUPRIMIR COMUNIDAD
      </button>
      <button @click="$emit('createPropietario')" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg transition">
        CREAR PROPIETARIOS
      </button>
      <button
